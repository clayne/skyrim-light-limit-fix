          ...
          uint  groupIndex          : SV_GroupIndex)
{
    ...
    float cluster_near = camera_near * pow(camera_far / camera_near, groupId.z / float(CLUSTER_BUILDING_DISPATCH_SIZE_Z));
    float cluster_far  = camera_near * pow(camera_far / camera_near, (groupId.z + 1) / float(CLUSTER_BUILDING_DISPATCH_SIZE_Z));
